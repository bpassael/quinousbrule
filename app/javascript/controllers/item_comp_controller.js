  ...
  selected(event) {
    ...
    if (this.totalCarb != 0) {
      ...
      if (this.totalJet < 1 || this.totalYacht < 2) {
        ...
        this.tweetTarget.href = `https://twitter.com/intent/tweet?text=Mes efforts réduisent mon empreinte carbone de ${Number.parseInt(this.totalCarb)} kgCO2e par an. Il me faudrait ${Math.round(Number.parseFloat(4500/this.totalCarb))} ans pour compenser 1 heure de vol dans un jet privé, ou 20km parcourus par un yacht. https://pic.twitter.com/XnIujtmLWg&hashtags=quinousbrule`
        this.tweettextTarget.innerText = `Mes efforts réduisent mon empreinte carbone de ${Number.parseInt(this.totalCarb)} kgCO2e par an. Il me faudrait ${Math.round(Number.parseFloat(4500/this.totalCarb))} ans pour compenser 1 heure de vol dans un jet privé, ou 20km parcourus par un yacht. https://pic.twitter.com/XnIujtmLWg&hashtags=quinousbrule`
        this.mailtextTarget.innerText = `Le site quinousbrule.fr m'a permis de réaliser que mes efforts réduisent mon empreinte carbone de ${Number.parseInt(this.totalCarb)} kgCO2e par an. Il me faudrait ${Math.round(Number.parseFloat(4500/this.totalCarb))} ans pour compenser 1 heure de vol dans un jet privé, ou 20km parcourus par un yacht.%0D%0A%0D%0AQue faites-vous pour lutter contre les vrais responsables du réchauffement climatique ?%0D%0A%0D%0ACordialement,%0D%0A%0D%0AVOTRE NOM`
        } else {
        this.tweetTarget.href = `https://twitter.com/intent/tweet?text=Mes efforts réduisent mon empreinte carbone de ${Number.parseInt(this.totalCarb)} kgCO2e par an. Ils sont annulés par ${timeConvert(this.totalJet)} de vol dans un jet privé, ou ${Math.round(this.totalYacht)}km parcourus par un yacht. A quoi servent-ils ? https://pic.twitter.com/XnIujtmLWg&hashtags=quinousbrule`
        this.tweettextTarget.innerText = `Mes efforts réduisent mon empreinte carbone de ${Number.parseInt(this.totalCarb)} kgCO2e par an. Ils sont annulés par ${timeConvert(this.totalJet)} de vol dans un jet privé, ou ${Math.round(this.totalYacht)}km parcourus par un yacht. A quoi servent-ils ? https://pic.twitter.com/XnIujtmLWg&hashtags=quinousbrule`
        this.mailtextTarget.innerText = `Le site quinousbrule.fr m'a permis de réaliser que mes efforts réduisent mon empreinte carbone de ${Number.parseInt(this.totalCarb)} kgCO2e par an. Ils sont annulés par la consommation carburant de ${timeConvert(this.totalJet)} de vol dans un jet privé, ou de ${this.totalYacht.toFixed(2)} kilomètres parcourus par un yacht.%0D%0A%0D%0AQue faites-vous pour lutter contre les vrais responsables du réchauffement climatique ?%0D%0A%0D%0ACordialement,%0D%0A%0D%0AVOTRE NOM`
